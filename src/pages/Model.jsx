...
export default function Model() {
  const [modelKey, setModelKey] = useState("lstm");
  ...
  const load = useCallback(async (key) => {
    ...
    try {
      ...
      setData({
        model: json?.model ?? MODEL_OPTIONS.find((o) => o.value === key)?.label,
        epochs: Array.isArray(json?.epochs) ? json.epochs : [],
        perClass: Array.isArray(json?.perClass) ? json.perClass : [],
        roc: Array.isArray(json?.roc) ? json.roc : [],
        auc: json?.auc ?? null,
        confusion: json?.confusion ?? null,
  ...
  return (
    <div className="cg-model">
      ...
      <Row gutter={[12, 12]}>
        <Col xs={24} lg={12}>
          <Card className="cg-card" title="Training vs. Validation Accuracy over Epochs" bordered={false}>
            {loading ? (
              <Skeleton active />
            ) : (
              <LineChart
        ...
        <Col xs={24} lg={12}>
          <Card className="cg-card" title="Training vs. Validation Loss over Epochs" bordered={false}>
            {loading ? (
              <Skeleton active />
            ) : (
              <LineChart
      ...
      {/* MID ROW: PRF | ROC | Confusion */}
      <Row gutter={[12, 12]} className="cg-model__row">
        <Col xs={24} lg={8}>
          <Card className="cg-card" title="Precision, Recall, F1-Score per Class" bordered={false}>
            {loading ? (
              <Skeleton active />
            ) : (
